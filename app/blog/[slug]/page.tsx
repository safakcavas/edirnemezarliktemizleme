...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!post) {
    return {
      title: "Yazı Bulunamadı | Edirne Mezar Bakım",
    };
  }

  const postUrl = `https://edirnemezarliktemizleme.site/blog/${post.slug}`;

  return {
    title: `${post.title} | Edirne Mezar Bakım`,
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    headline: post.title,
    description: post.description,
    image: `https://edirnemezarliktemizleme.site${post.image}`,
    ...
    mainEntityOfPage: {
      "@type": "WebPage",
      "@id": `https://edirnemezarliktemizleme.site/blog/${post.slug}`,
    },
  };
    ...
    itemListElement: [
      {
        ...
        position: 1,
        name: "Ana Sayfa",
        item: "https://edirnemezarliktemizleme.site",
      },
      {
        "@type": "ListItem",
        position: 2,
        name: "Blog",
        item: "https://edirnemezarliktemizleme.site/blog",
      },
      {
        "@type": "ListItem",
        position: 3,
        name: post.title,
        item: `https://edirnemezarliktemizleme.site/blog/${post.slug}`,
      },
    ],
  };
          ...
          <div className="relative rounded-3xl overflow-hidden aspect-[16/9] mb-12 shadow-lg border border-slate-200">
            ...
            <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between text-white text-xs">
              <span className="bg-slate-900/80 backdrop-blur px-3 py-1 rounded-lg">
                Edirne Mezar Bakım Saha Çalışması
              </span>
              <span className="text-slate-200">9 İlçe & 253 Köy Mezarlığı</span>
            </div>
          </div>
          ...
          <div className="mt-12 p-6 sm:p-8 rounded-3xl bg-gradient-to-br from-emerald-800 to-teal-950 text-white shadow-xl flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="space-y-2 text-center md:text-left">
              ...
              <h3 className="text-xl sm:text-2xl font-bold text-white">
                Edirne’deki Kabir İçin Fiyat ve Bilgi Alın
              </h3>
              <p className="text-xs sm:text-sm text-slate-300 max-w-md">
                Edirne Merkez ve tüm 8 ilçedeki 253 köye kendi su depomuzla ulaşıyor, kabri bulup videolu raporluyoruz.
              </p>
            </div>
