...
export default function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const formatted = [
      "🌿 *EDİRNE MEZAR TEMİZLEME İLETİŞİM FORMU*",
      `👤 *İsim:* ${name.trim() || "Belirtilmedi"}`,
      `📞 *Telefon:* ${phone.trim() || "Belirtilmedi"}`,
      `📍 *İlçe/Köy:* ${district.trim() || "Belirtilmedi"}`,
      `💬 *Mesaj:* ${message.trim() || "Genel bilgi talebi"}`,
    ].join("\n");

    const url = contactConfig.getWhatsappUrl(formatted);
    setSubmitted(true);
    window.open(url, "_blank");
  };

  return (
    <section id="iletisim" className="py-20 bg-slate-900 text-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          {/* Contact Info Column */}
          <div className="lg:col-span-6 space-y-8">
            <div>
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-900/80 border border-emerald-700/50 text-emerald-300 text-xs font-bold uppercase tracking-wider mb-4">
              ...
              <p className="text-slate-300 mt-3 text-base leading-relaxed">
                Edirne ve tüm köylerindeki mezarlık bakım, mermer beyazlatma,
